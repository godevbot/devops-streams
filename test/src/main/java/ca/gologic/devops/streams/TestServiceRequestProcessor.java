...
public class TestServiceRequestProcessor implements StreamProcessorThread {
	private static Logger LOG = LoggerFactory.getLogger(TestServiceRequestProcessor.class);
		...
		try (final KafkaConsumer<String, ServiceEvent> consumer = new KafkaConsumer<>(consumerProps)) {
			...
			while (running) {
				...
				for (final ConsumerRecord<String, ServiceEvent> consumerRecord : records) {
					...
					if (serviceEvent.getSubject().toString().equalsIgnoreCase("test")) {
